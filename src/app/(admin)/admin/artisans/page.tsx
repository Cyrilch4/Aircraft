...
import { UserPlus, ExternalLink, CheckCircle2, Clock, XCircle, Network, LogIn, Building2 } from 'lucide-react'
...
type ArtisanType = 'independant' | 'entreprise'
...
const ARTISANS = [
  {
    ...
    email: 'sophie.martin@email.fr',  tel: '06 12 34 56 78',
    adresse: '12 rue du Faubourg, 75011 Paris',
    projets: 8,  convention: 'signee',
  },
  {
    ...
    email: 'thomas.dupont@email.fr',  tel: '06 23 45 67 89',
    adresse: '3 allée des Chênes, 69007 Lyon',
    projets: 5,  convention: 'signee',
  },
  {
    ...
    email: 'julie.bernard@email.fr',  tel: '06 34 56 78 90',
    adresse: '8 avenue Gambetta, 33000 Bordeaux',
    projets: 3,  convention: 'attente',
  },
  {
    ...
    nb_salaries: 9,
    activites: ['maconnerie', 'peinture', 'couverture'] as ActiviteSlug[],
    email: 'marc.leroy@email.fr',     tel: '06 45 67 89 01',
    adresse: '27 rue Nationale, 59000 Lille',
    projets: 12, convention: 'signee',
  },
  {
    ...
    email: 'emma.moreau@email.fr',    tel: '06 56 78 90 12',
    adresse: '5 place du Capitole, 31000 Toulouse',
    projets: 1,  convention: 'non_signee',
  },
]

const CONVENTION_CONFIG = {
  signee:     { label: 'Signée',      icon: CheckCircle2, className: 'text-[#34C759] bg-[#34C759]/10' },
  attente:    { label: 'En attente',  icon: Clock,        className: 'text-[#FF9500] bg-[#FF9500]/10' },
  non_signee: { label: 'Non signée',  icon: XCircle,      className: 'text-[#FF3B30] bg-[#FF3B30]/10' },
}
...
export default async function ArtisansPage() {
  ...
  const entreprises = ARTISANS.filter((a) => a.type === 'entreprise').length

  const galaxies = await Promise.all(
    ARTISANS.map((a) =>
      getLevelCounts(a.id)
        .then((counts) => counts.reduce((sum, c) => sum + c.count, 0))
        .catch(() => 0)
    )
  )
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="mt-1 text-[15px] text-[#6E6E73]">
            {ARTISANS.length} professionnels · {entreprises} entreprise{entreprises > 1 ? 's' : ''} · {signees} conventions signées
          </p>
        </div>
                ...
                <tr key={a.id} className="hover:bg-[#F5F5F7] transition-colors group">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-[14px] text-[#1D1D1F]">{a.prenom} {a.nom}</div>
                        {a.type === 'entreprise' ? (
                          <div className="flex items-center gap-1 mt-0.5">
                            <Building2 className="w-3 h-3 text-[#6E6E73]" />
                            <span className="text-[11px] text-[#6E6E73]">{a.nom_entreprise} · {a.nb_salaries} sal.</span>
                          </div>
                        ) : (
                          <div className="text-[11px] text-[#6E6E73] mt-0.5">Indépendant</div>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <ActiviteList slugs={a.activites} />
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <span className="text-[14px] font-semibold text-[#AF52DE]">{galaxies[i]}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4">
